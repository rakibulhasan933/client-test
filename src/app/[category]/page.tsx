...
export async function generateMetadata(
    ...
    const item = await subCategoryList?.filter((item) => item.name === subcategory);
    const locality = searchParams.locality;
    const state = searchParams.state;
    return {
        title: locality ? `${params.category}  ${item?.at(0)?.name} in ${locality}, ${state}  Nigeria  ` : `${params.category} in Nigeria`,
    }
}
